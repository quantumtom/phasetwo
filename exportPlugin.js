...
module.exports = class ExportPlugin {
    apply(compiler) {
        compiler.plugin('emit', (compilation, done) => {
            ...
            const footerTemplate = `.then(function(mod) { resolve(mod.default.start({el: options.el, config: options.config, lightboxController: options.lightboxController}))}); }); } }; });`
            compilation.chunks.forEach(chunk => {
            console.log('checkit ================', chunk.files[0]);
